...
class NewsThirdFragment : Fragment() {

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
                              savedInstanceState: Bundle?): View? {
        return inflater.inflate(R.layout.fragment_news_third, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        val recyclerView = view.findViewById<RecyclerView>(R.id.third_recycler_list)
        val adapter = NewsRecyclerViewAdapter(createDataList(), object : NewsRecyclerViewAdapter.ListListener {
            override fun onClickListItem(tappedView: View, article: Article) {
                this@NewsThirdFragment.onClickListItem(tappedView, article)
            }
        })
    ...
    private fun createDataList(): List<Article> {
        return arguments!!.getSerializable("third_list") as MutableList<Article>
    }
